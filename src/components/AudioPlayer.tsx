...
export default function AudioPlayer({ fileId, fileUrl }: AudioPlayerProps) {
  ...
  const fetchNotes = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/memories/${fileId}`, {
  ...
  const handleSendNote = async () => {
    ...
    try {
      const token = session?.accessToken;
      await axios.post('http://localhost:5000/api/memories/note', {
        fileId: fileId,
  ...
  const handleDeleteNote = async (noteId: string) => {
    ...
    try {
      const token = session?.accessToken;
      await axios.delete(`http://localhost:5000/api/memories/note/${noteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchNotes();
    } catch (error) {
  ...
  return (
    <div className="w-full bg-gray-900 p-4 rounded-xl border border-gray-700">
      ...
      <div className="flex items-center gap-4 mb-4">
        <button onClick={togglePlay} className="w-12 h-12 bg-indigo-600 rounded-full flex items-center justify-center hover:bg-indigo-500 transition">
      ...
      <div className="flex gap-2 mb-6">
        ...
        />
        <button onClick={handleSendNote} className="bg-indigo-600 px-4 rounded hover:bg-indigo-500 transition">
          <FaPaperPlane />
        </button>
      </div>
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-600">
        {notes.sort((a,b) => a.timestamp - b.timestamp).map((note) => {
            ...
            return (
              <div 
                key={note.id} 
                className={`flex justify-between items-center p-2 rounded text-sm transition-all ${isActive ? 'bg-indigo-900 border-l-4 border-indigo-400' : 'bg-gray-800'}`}
              >
                <div 
                    className="cursor-pointer flex-1"
                    onClick={() => {
                        if(audioRef.current) audioRef.current.currentTime = note.timestamp;
                    }}
                >
                  <span className="text-indigo-400 font-bold mr-2">[{formatTime(note.timestamp)}]</span>
                  <span className="text-gray-200">{note.content}</span>
                </div>
                <button onClick={() => handleDeleteNote(note.id)} className="text-red-500 hover:text-red-400 ml-2">
                  <FaTrash size={12} />
                </button>
              </div>
            );
